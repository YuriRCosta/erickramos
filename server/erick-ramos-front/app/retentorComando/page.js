...
export default function RetentorComando() {
    ...
    const handlePesquisar = () => {
        if (nomePesquisa) {
            Api
                .get(`/retentorComando/nome/${nomePesquisa}`, {
                    headers: {
                        Authorization: `Bearer ${accessToken}`
                    }
                })
    ...
    async function deleteRetentorComando(id) {
        ...
        try {
            await Api.delete(`retentorComando/${id}`, {
                headers: {
                    Authorization: `Bearer ${accessToken}`
                }
            });
            setRetentoresComando(retentoresComando.filter(selo => selo.id !== id));
        } catch (e) {
            alert('Falha ao deletar retentor de comando, tente novamente.');
        }
    }
