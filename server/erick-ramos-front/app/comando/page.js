...
export default function Comando() {
    ...
    async function deleteComando(id) {
        ...
        try {
            await Api.delete(`junta/${id}`, {
                headers: {
                    Authorization: `Bearer ${accessToken}`
                }
            });
            setComandos(comandos.filter(selo => selo.id !== id));
        } catch (e) {
                ...
                <div className="max-w-md w-full p-6 bg-neutral-500 rounded-lg shadow-md text-center">
                    ...
                    <table className="w-full">
                        ...
                        <tr>
                            ...
                            <th className="py-2 text-center px-4">Exlcuir</th>
                        </tr>
                        </thead>
                        <tbody>
                        {comandos.map((retentor) => (
                            <tr key={retentor.id}>
                                ...
                                <button
                                    ...
                                    onClick={() => deleteComando(retentor.id)}
                                >
                                    <svg
